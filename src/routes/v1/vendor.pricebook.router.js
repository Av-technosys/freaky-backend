    ...
    const vendorAllPriceBooks = await db
      ...
      .where(
        and(
          ...
          eq(priceBookEntry.priceBookingId, priceBookId)
        )
      );
...
pricebookRouter.put('/update', checkVendor, async (req, res) => {
  try {
    ...
    if (!Array.isArray(updates)) {
      return res
        .status(400)
        .send({ message: 'Invalid payload, expected an array of updates' });
    }
  ...
  try {
    ...
    if (isActive) {
      await db
        ...
        .where(
          and(eq(priceBook.vendorId, vendorId), eq(priceBook.isStandard, true))
        );

      await db
        .update(priceBook)
        .set({ isStandard: true })
        .where(eq(priceBook.id, newPriceBook.id));
    }

    return res.send({
      msg: 'Pricebook created successfully',
      data: newPriceBook,
    });
  } catch (error) {
    console.error('Error: ', error);
...
pricebookRouter.delete('/:pricebookId', checkVendor, async (req, res) => {
  ...
  try {
    ...
    const [deletePricebookData] = await db
      .select()
      .from(priceBook)
      .where(
    ...
    if (deletePricebookData.isActive || deletePricebookData.isStandard) {
      return res
        .status(400)
        .send({ message: 'Pricebook is active or standard' });
    }
    await db
